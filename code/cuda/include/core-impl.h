    ...
    for (int epoch = 0; epoch < maxEpochs; epoch++)
    {
        ...
        for (int batchStart = 0; batchStart < numSamples; batchStart += batchSize)
        {
            ...
            CHECK_CUBLAS_ERROR(cublasSgemm(cublasHandle,
                                           ...
                                           &alpha,
                                           d_weights, 1,
                                           d_features + batchStart * numFeatures, numFeatures,
                                           &beta,
                                           d_predictions, 1););

            // 2. Compute errors (predictions - y_batch)
            CHECK_CUBLAS_ERROR(cublasSaxpy(cublasHandle, currentBatchSize, &neg_alpha, d_labels + batchStart, 1, d_predictions, 1));
            ...
            CHECK_CUBLAS_ERROR(cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T, 1, numFeatures, currentBatchSize,
                                           &alpha, d_predictions, 1, d_features + batchStart * numFeatures, numFeatures, &beta, d_gradients, 1));
...
void CudaSGD::benchmark()
{
    ...
    // warp, async
    std::vector<std::string> methods = {"cublas", "custom", "coalesced"};
